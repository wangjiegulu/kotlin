...
class ScriptingCompilerConfigurationExtension(val project: MockProject) : CompilerConfigurationExtension {
    ...
    override fun updateConfiguration(configuration: CompilerConfiguration) {
        ...
        if (scriptDefinitions.isNotEmpty()) {
            ...
            scriptResolverEnv["projectRoot"] = project.run { basePath ?: baseDir?.canonicalPath }?.let(::File)
            configureScriptDefinitions(scriptDefinitions, configuration, messageCollector, scriptResolverEnv)
        }
    }
}
